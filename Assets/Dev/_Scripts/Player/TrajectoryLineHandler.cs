...
public class TrajectoryLineHandler : MonoBehaviour
{
    ...
    public void DrawTrajectoryLine(Vector3 position, Vector3 direction)
    {
        ...
        if (Physics.SphereCast(ray, 0.5f, out hit, _currentDistance, targetLayer))
        {
            if (hit.transform.gameObject.CompareTag("Obstacle"))
            {
                // Get reflect for direction
                direction = Vector3.Reflect(direction, hit.normal);
                position = hit.point;
                _currentDistance -= Vector3.Distance(startingPosition, position);
            }
            else if (hit.transform.gameObject.CompareTag("TrajectoryHelper"))
            {
                // Align direction's rotation with trajectory helper object
                direction = Quaternion.FromToRotation(Vector3.back, hit.normal) * direction;
                position = hit.point;
                _currentDistance -= Vector3.Distance(startingPosition, position);
